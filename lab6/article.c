...
int main()
{
      ...
      int st;
   	...
   	for (int i = 0 ; i < 100000000; ++i)
   	{
         ...
         fprintf(fd, "{ \" UNIQUE_KEY\" : %d, \"text\": \"s/text/%c%c%c%c%c.txt\", \"pict\" : \"s/pict/%c%c%c%c%c.png\" , \"script\" : \"s/scr/%c%c%c%c%c\"}\t", 
            rand()
         ...
         fprintf(fd, "\" #%ld#;)# %s %s %s \"\t",  //text
         random(),PREFIX[rand()%6],GOOD_WORD[ (rand() % 10) ], POSTFIX[(rand() % 24)] );

         fprintf(fd,"%d\t", rand()%1000000+1);//id

         int y;
         fprintf(fd,"\'%d-%d-%d %d:%d:%d\'\t", 
                     y=rand()%2000+1000,rand()%12+1,rand()%27+1,rand()%24,rand()%60,rand()%60); //time_creation
