...
CREATE TABLE EMPLOYEE (
  employee_id SMALLINT NOT NULL PRIMARY KEY,    -- Код сотрудника
  last_name VARCHAR(15),    -- Фамилия
  first_name VARCHAR(15),   -- Имя
  middle_initial VARCHAR(1),    -- Средний инициал
  manager_id SMALLINT,  -- Код начальника
  job_id SMALLINT,  -- Код должности
  hire_date TIMESTAMP,  -- Дата поступления в фирму
  salary NUMERIC(7,2),  -- Зарплата
  commission NUMERIC(7,2),  -- Комиссионные
  department_id SMALLINT  );    -- Код отдела

-- Таблица DEPARTMENT - отделы фирмы --
CREATE TABLE DEPARTMENT (
  department_id SMALLINT NOT NULL PRIMARY KEY,  -- Код отдела
  name VARCHAR(14), -- Название отдела
  location_id SMALLINT );   -- Код места размещения

-- Таблица LOCATION - места размещения отделов --
CREATE TABLE LOCATION (
  location_id SMALLINT NOT NULL PRIMARY KEY,    -- Код места размещения
  regional_group VARCHAR(20) ); -- Город

-- Таблица JOB - должности в фирме --
CREATE TABLE JOB  (
  job_id SMALLINT NOT NULL PRIMARY KEY, -- Код должности
  "function" VARCHAR(30) ); -- Название должности

-- Таблица CUSTOMER - фирмы-покупатели --
CREATE TABLE CUSTOMER (
  customer_id INTEGER NOT NULL PRIMARY KEY, -- Код покупателя
  name VARCHAR(45), -- Название покупателя
  address VARCHAR(40),  -- Адрес
  city VARCHAR(30), -- Город
  state VARCHAR(2), -- Штат
  zip_code VARCHAR(9),  -- Почтовый код
  area_code SMALLINT,   -- Код региона
  phone_number INTEGER, -- Телефон
  salesperson_id SMALLINT,  -- Код сотрудника-продавца, обслуживаю-щего данного покупателя
  credit_limit NUMERIC(9,2),    -- Кредит для покупателя
  comments VARCHAR(500) );  -- Примечания

-- Таблица SALES_ORDER - договоры о продаже --
CREATE TABLE SALES_ORDER (
  order_id SMALLINT NOT NULL PRIMARY KEY,   -- Код договора
  order_date DATE,  -- Дата договора
  customer_id INTEGER,  -- Код покупателя
  ship_date DATE,   -- Дата поставки
  total NUMERIC(8,2) ); -- Общая сумма договора

-- Таблица ITEM - акты продаж --
CREATE TABLE ITEM (
  order_id SMALLINT NOT NULL,   -- Код договора, в состав которого входит акт
  item_id SMALLINT NOT NULL,    -- Код акта
  product_id INTEGER,   -- Код продукта
  actual_price NUMERIC(8,2),    -- Цена продажи
  quantity INTEGER, -- Количество
  total NUMERIC(8,2),   -- Общая сумма
PRIMARY KEY (order_id,item_id)
);

-- Таблица PRODUCT - товары --
CREATE TABLE PRODUCT (
  product_id INTEGER NOT NULL PRIMARY KEY,  -- Код продукта
  description VARCHAR(30) );    -- Название продукта

-- Таблица PRICE - цены --
CREATE TABLE PRICE (
  product_id INTEGER NOT NULL,  -- Код продукта
  list_price NUMERIC(8,2),  -- Объявленная цена
  min_price NUMERIC(8,2),   -- Минимально возможная цена
  start_date DATE NOT NULL, -- Дата установления цены
  end_date DATE,    -- Дата отмены цены
